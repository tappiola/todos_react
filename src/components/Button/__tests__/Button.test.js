...
describe('<Button/>', () => {
    ...
    it("should pass additional classes to <button/>", () => {
        const wrapper = shallow(<Button className={clsx("class1", "class2")}>Click me</Button>);
        const buttonWrapper = wrapper.find("button");
    ...
    it("should pass additional props to <button/>", () => {
        const wrapper = shallow(<Button newprop="propValue">Click me</Button>);
        const buttonWrapper = wrapper.find("button");
...
describe('<DeleteButton/>', () => {
    it("should pass additional classes to <button/>", () => {
        const wrapper = mount(<DeleteButton className={clsx("class1", "class2")}>Click me</DeleteButton>);
        const buttonWrapper = wrapper.find("button");
        expect(buttonWrapper.get(0).props.className.split(' ')).toEqual(expect.arrayContaining(["class1", "class2"]));
    });
...
describe('<CancelButton/>', () => {
    it("should pass additional classes to <button/>", () => {
        const wrapper = mount(<CancelButton className={clsx("class1", "class2")}>Click me</CancelButton>);
        const buttonWrapper = wrapper.find("button");
        expect(buttonWrapper.get(0).props.className.split(' ')).toEqual(expect.arrayContaining(["class1", "class2"]));
    });
